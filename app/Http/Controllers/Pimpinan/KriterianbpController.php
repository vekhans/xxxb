...
class KriterianbpController extends Controller
{
    /**
    ...
    public function index($periodes, $devisis)
    {
        try
        {
            ...
            if (Auth::User()->id==2){
                ...
                return view('pimpinan.kriterianb.index',['title' => $title,'periodes'=>$periodes,'devisis'=>$devisis])->with($params);
            }
            else{
                return redirect()->route('login')->with('warning','Data tidak valid!');
            }
    ...
    public function update(Request $request,$periodes, $devisis, string $id = null)
    {
        if (Auth::User()->id== 2){
            ...
            if(!$devisis)
            {
                return redirect()->route('devisis.index',[$periodes])->with('warning','Data tidak valid!');
            }
            try
            {
                ...
                foreach ($kriterianabsss as $keyss => $valaue) {
                 ...
                 return redirect()->route('kriterianbp.index',[$periodes->id, $devisis->id])->with('info', "Data berhasil diubah.");
             }
            catch (ModelNotFoundException $ex)
            {
                if ($ex instanceof ModelNotFoundException)
