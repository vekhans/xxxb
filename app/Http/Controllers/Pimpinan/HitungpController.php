...
class HitungpController extends Controller
{
    public function index($periodes)
    {
        ...
        try
        {
            ...
            if (Auth::User()->kategori== 'Pimpinan'){
                ...
                if ($datardff < 3) { 
                    return redirect()->route('periodes.index')->with('warning','Silahkan Input Minimal 3 Kriteria pada Devisi Cuci!');
                }
                $datardffstrika = Kriteria::where('devisi','=',$devisiss->id)->count();
                if ($datardffstrika < 3) { 
                    return redirect()->route('periodes.index')->with('warning','Silahkan Input Minimal 3 Kriteria pada Devisi Strika!');
                }
                $datardffpacking = Kriteria::where('devisi','=',$devisisp->id)->count();
                if ($datardffpacking < 3) { 
                    return redirect()->route('periodes.index')->with('warning','Silahkan Input Minimal 3 Kriteria pada Devisi Packing!');
                }

                $datardssffc = Rank::all()->where('devisi','=',$devisisc->id)->where('periode','=',$periodes->id)->count();
                if ($datardssffc < 2) { 
                    return redirect()->route('periodes.index')->with('warning','Silahkan Input Minimal 2 Alternatif pada devisi Cuci!');
                }
                $datardssffs = Rank::all()->where('devisi','=',$devisiss->id)->where('periode','=',$periodes->id)->count();
                if ($datardssffs < 2) { 
                    return redirect()->route('periodes.index')->with('warning','Silahkan Input Minimal 2 Alternatif pada devisi Strika!');
                }
                $datardssffp = Rank::all()->where('devisi','=',$devisisp->id)->where('periode','=',$periodes->id)->count();
                if ($datardssffp < 2) { 
                    return redirect()->route('periodes.index')->with('warning','Silahkan Input Minimal 2 Alternatif pada devisi Packing!');
                }
                $datassrdssffc = Alternatifnb::all()->where('periode','=',$periodes->id);
                foreach ($datassrdssffc as $key ) {
                    if ($key->nilai < 1) {
                        return redirect()->route('periodes.index')->with('warning','Silahkan atur Nilai Bobot Alternatif!');
                    }
                } 
